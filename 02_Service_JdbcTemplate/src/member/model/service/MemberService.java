...
public class MemberService {
	...
	public int insertMember(Member member) {		
		int result = 0;
		// 1. Connection 생성하는 메서드 불러서 Connction 가져오기
	...
	public int updateMemberName(Member member) {
		...
		try {
			// 2. dao 요청
			result = memberDao.updateMemberName(conn, member);
	...
	public int updateMemberBirthday(Member member) {
		...
		try {
			// 2. dao 실행
			result = memberDao.updateMemberBirthday(conn, member);
			// 3-1. 트랜잭션 처리
			commit(conn);
		} catch (Exception e) {
			// 3-2. 트랜잭션 처리
	...
	public int updateMemberEmail(Member member) {
		...
		try {
			// 2. dao 실행
			result = memberDao.updateMemberEmail(conn, member);
			// 3-1. 트랜잭션 처리
			commit(conn);
		} catch (Exception e) {
			// 3-2. 트랜잭션 처리
	...
	public int updateMemberAddress(Member member) {
		...
		try {
			// 2. dao 실행 
			result = memberDao.updateMemberAddress(conn, member);
